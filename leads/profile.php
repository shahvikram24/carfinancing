    ...
    if($imageupdate=="on" && isset($mode) && $mode=="addimage")
    {
      ...
      { 
        ...
        if (strlen($filename) > 0)
        {
          ...
          header("Location:".LEADASSIGNURL . 'profile.php?' . $Encrypt->encrypt('ContactId='.$ContactId."&Message=".$Message));
          exit;
        }
              ...
              header("Location:".LEADASSIGNURL . 'profile.php?' . $Encrypt->encrypt('ContactId='.$ContactId."&Message=".$Message."&Success=true"));
    }
}
