            ...
            $dealeremail = dealership::GetDealerEmail($DealerId);
        	 $mailObj = new Email($dealeremail, "New Lead CarFinancing.Help  ", "Email - ". FormatInitCap(FormatName($Contact->first_name,$Contact->last_name)));
                ...
                try 
                {
                     ...
                     if($mailObj->Send())
                     {
                        header("Location:".LEADASSIGNURL . 'new-leads.php?' . $Encrypt->encrypt('Message=Dealer assigned successfully to the contact. Informaion has been sent to dealer.&Success=true&ContactId='.$ContactId));    	
	    				exit();
                    }

                    else 
                    {
                    	header("Location:".LEADASSIGNURL . 'new-leads.php?' . $Encrypt->encrypt('Message=Something went wrong while sending email.&Success=false&ContactId='.$ContactId));    	
    	 				exit();
                    }
                }
                catch (Exception $e) {
